...
namespace :win do
  ...
  task :upload => :sign do
    ...
    sh("aws s3 sync #{'--no-progress' unless $stdin.tty?} --acl public-read --cache-control 'max-age=31536000' #{signing_dir} s3://#{S3_DOWNLOAD_BUCKET_BASE_URL}/binaries/#{go_full_version}/win")
  end
end
