...
class AddonMetadata
  ...
  def upload_combined_metadata_file
    ...
    sh("aws s3 cp #{@combined_metadata_file_location.path} #{@metadata_full_s3_path}")
  end

  private

  def fetch_existing_metadata
    Tempfile.open(%w(existing-metadata .json)) do |old_json|
      sh("aws s3 cp #{@metadata_full_s3_path} #{old_json.path}")
